...
#define TASK_SW180_ALARM_HOLD_MS 2000
...
static void task_sw180_entry(void *arg)
{
    ...
    uint32_t alarm_hold_until_ms = 0U;
    ...
    for (;;) {
        ...
        if (irq_fired && (level == SAVEBOX_SW180_TRIGGER_LEVEL) && ((now_ms - last_irq_ms) >= TASK_SW180_DEBOUNCE_MS)) {
            ...
            last_irq_ms = now_ms;
            alarm_active = true;
            alarm_hold_until_ms = now_ms + TASK_SW180_ALARM_HOLD_MS;
        }

        if (alarm_active && (now_ms >= alarm_hold_until_ms) && (level != SAVEBOX_SW180_TRIGGER_LEVEL)) {
            UART_Printf(&huart1, "SW180 DO=%d\r\n", (int)level);
            UART_Printf(&huart1, "Status Normal\r\n");
            alarm_active = false;
        }

        task_state_set_vibration((int)level, alarm_active);
    }
}
